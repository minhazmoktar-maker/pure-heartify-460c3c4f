    ...
    <div className="min-h-dvh bg-background">
      ...
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        <header className="mt-4 mb-6">
          <div className="flex items-center gap-3"><Droplet className="h-7 w-7 text-primary" /><h1 className="text-title font-bold">Women's Fiqh</h1></div>
        ...
        <div className="grid gap-4">
          {filtered.map(r => (
            <Card key={r.id} className={`p-5 cursor-pointer transition ${read[r.id] ? "bg-primary/5 border-primary/40" : "hover:bg-muted/40"}`} onClick={() => persist({ ...read, [r.id]: !read[r.id] })}>
              <div className="flex items-start justify-between gap-4"><div><h2 className="text-heading font-semibold">{r.title}</h2><Badge variant="outline" className="mt-1">{r.topic}</Badge></div>{read[r.id] && <Badge>Read</Badge>}</div>
        ...
        <p className="mt-6 text-micro text-muted-foreground text-center">Educational summary — consult a qualified scholar for personal cases.</p>
      </div>
    </div>
  );
};
