...
const SunnahPrayers = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b bg-card"><div className="container mx-auto px-4 py-4 flex items-center gap-3"><Link to="/"><Button variant="ghost" size="icon"><ArrowLeft className="w-5 h-5" /></Button></Link><Moon className="w-6 h-6 text-primary" /><h1 className="text-2xl font-bold">Sunnah & Nafl Prayers</h1></div></div>
      <div className="container mx-auto px-4 py-6 space-y-4">
        ...
        <div className="flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input value={q} onChange={e => setQ(e.target.value)} placeholder="Search prayers…" className="pl-9" /></div><Button variant="outline" size="icon" onClick={() => persist({})} title="Reset"><RotateCcw className="w-4 h-4" /></Button></div>
