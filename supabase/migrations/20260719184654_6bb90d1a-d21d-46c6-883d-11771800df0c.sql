...
BEGIN
  ...
  ch_stats AS (
    SELECT r,
           ...
           ROUND(SUM(-((cnt/tot)) * LN(GREATEST(1e-9,(cnt/tot))))/LN(2),3) AS entropy_bits
    FROM ch_freq GROUP BY r
  )
...
BEGIN
  ...
  ch_ent AS (
    SELECT ROUND(SUM(-((c/SUM(c) OVER ())) * LN(GREATEST(1e-9,(c/SUM(c) OVER ()))))/LN(2),3) e FROM ch
  ),
  ch_e AS (SELECT ROUND(AVG(e),3) e FROM ch_ent)
  SELECT
    COUNT(*)::int,
    COUNT(DISTINCT video_id)::int,
    COUNT(DISTINCT channel_title)::int,
    COUNT(DISTINCT category)::int,
    (SELECT ROUND(100.0*MAX(c)/NULLIF(SUM(c),0),2) FROM ch),
    (SELECT ROUND(100.0*MAX(c)/NULLIF(SUM(c),0),2) FROM ca),
    ROUND(100.0 * (COUNT(*) - COUNT(DISTINCT video_id))::numeric / NULLIF(COUNT(*),0), 2),
    (SELECT e FROM ch_e),
    ROUND(100.0*COUNT(*) FILTER (WHERE published_at > now()-interval '7 days')::numeric/NULLIF(COUNT(*),0),2),
    ROUND(100.0*COUNT(*) FILTER (WHERE is_trusted_channel)::numeric/NULLIF(COUNT(*),0),2),
    ROUND(100.0*COUNT(*) FILTER (WHERE user_id IS NOT NULL)::numeric/NULLIF(COUNT(*),0),2)
  FROM imp;
END; $$;
