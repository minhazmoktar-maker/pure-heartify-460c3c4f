-- The stats-protection trigger was also blocking the trusted streak RPC
-- (SECURITY DEFINER still runs as the calling auth role), so streaks could
-- never advance past 1. Allow writes only when the trusted RPC sets a
-- transaction-local flag; direct client updates remain blocked.
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role'
     AND coalesce(current_setting('app.streak_rpc', true), '') <> 'on' THEN
...
BEGIN
  ...
  -- Trusted-write flag for protect_streak_stats (transaction-local).
