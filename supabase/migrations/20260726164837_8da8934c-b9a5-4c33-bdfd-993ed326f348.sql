-- One-time repair: the protection trigger blocked streak advances, so stored
-- counts are stuck at 1. Rebuild them from real activity days.
WITH days AS (
...
SET current_streak = GREATEST(s.current_streak, a.current),
    longest_streak = GREATEST(s.longest_streak, a.longest, GREATEST(s.current_streak, a.current)),
...
SELECT a.user_id, a.current, GREATEST(a.longest, a.current), a.last_day, a.total_days
FROM (
  WITH days AS (
    SELECT user_id, (watched_at AT TIME ZONE 'utc')::date AS d FROM public.watch_history
    UNION
...
WHERE NOT EXISTS (SELECT 1 FROM public.streaks s WHERE s.user_id = a.user_id);