...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Timezone-aware "today": use the user's local calendar date so a device
    // in UTC+13/UTC-11 completes the right dose and never loses a streak day
    // to a UTC rollover. Falls back to the UTC date if the tz is unknown.
    const utcToday = new Date().toISOString().slice(0, 10);
    const { data: localDate } = await admin.rpc("user_local_date", { _uid: userId });
    const today = typeof localDate === "string" && localDate ? localDate : utcToday;

    // Find today's dose containing this video. Accept the adjacent UTC date too,
    // for doses generated by the UTC-scheduled generator.
    const { data: doses } = await admin
      .from("daily_dose")
      .select("*")
      .eq("user_id", userId)
      .in("dose_date", [...new Set([today, utcToday])])
      .order("dose_date", { ascending: false })
      .limit(2);
    const dose =
      (doses ?? []).find((d) => (d.video_ids as string[])?.includes(videoId)) ?? null;
