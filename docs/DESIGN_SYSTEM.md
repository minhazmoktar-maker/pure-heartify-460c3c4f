...
- Off-scale radii (`rounded-sm`, `rounded-md`, `rounded-lg`, `rounded-xl`, `rounded-2xl`, `rounded-3xl`)
- Arbitrary durations (`duration-[…ms]`, `duration-75`, `duration-100`, `duration-150`, `duration-300`, `duration-500`, `duration-700`, `duration-1000`)

`prose-heartify` and the `heartify-*` component classes are allow-listed because they encapsulate their own scale.

Run locally: `node scripts/design-lint.mjs`.
