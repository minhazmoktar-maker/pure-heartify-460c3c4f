...
| MVP-5 | Shipped | Benefit-ranked feed behind `feed.benefit_ranked` (10% deterministic bucket). `benefit_priors_v1()` computes Bayesian-shrunk worth-it priors per channel and category; `_shared/surfaces/benefit.ts` re-ranks (0.7 relevance rank + 0.3 benefit delta), annotates honest reason chips above evidence thresholds, and traces the arm. Guardrail readout: `benefit_arm_readout()` on `/admin/benefit-labels` (treatment vs control response, worth-it and acted-on rates). |
| MVP-6 | Shipped | Guided learning paths on the graph: `learning_paths` / `learning_path_steps` / `learning_path_progress` with public read of published paths and per-user progress only. Four starter paths (creed, Qur'an, character, Seerah), 8 prerequisite-ordered steps each, every step backed by ≥2 reviewed lessons. Surfaces: `Guided paths` on `/learn` and `/learn/path/:slug` (progress bar, next-up step, prerequisite hints, optimistic complete/uncomplete via `set_learning_step_progress`) with `Course` JSON-LD. |
| MVP-7 | Next | Institutional co-sign: one university or dar al-ifta signing attestations in the ledger. |
